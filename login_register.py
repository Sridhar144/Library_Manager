	...
	# placing
     ...
     def login_admin(self):
        if len(self.namee.get()) == 0:
            ...
            token = login_backend.check(self.namee_text.get(), self.password1e_text.get())
            if token:
                messagebox.showinfo("Success", f"Admin Login Successful! Token: {token}")
                # Proceed with your admin interface, passing the token if necessary
     ...
     def login_student(self):
        if len(self.namee.get()) == 0:
            ...
            token = login_backend.checks(self.namee_text.get(), self.password1e_text.get())
            if token:
                messagebox.showinfo("Success", f"Student Login Successful! Token: {token}")
                # Proceed with your student interface, passing the token if necessary
     def destroy(self):
         self.labelr.destroy()
         self.namer.destroy()
         self.namere.destroy()
